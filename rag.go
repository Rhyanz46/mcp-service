...
func loadDocsWithConfig(dir string, config *Config) ([]Doc, error) {
	// Use chunker to get documents
	chunks, err := makeChunks(dir, config.Indexing.ChunkSize, config.Indexing.ChunkOverlap, config.Indexing.IncludeCode, config)
	if err != nil {
		return nil, err
	}
...
func buildIndex(docs []Doc, config *Config) *Inverted {
	idx := &Inverted{
		Docs:    docs,
...
func (idx *Inverted) Search(query string, k int) []Hit {
	...
	// candidate docs: gabungan posting list kata-kata query
	...
	// skor gabungan BM25 + cosine (alpha)
	const alpha = 0.2
...
func LoadIndexFromConfig(config *Config) (*Inverted, error) {
