...
func NewOpenAIProviderWithConfig(config *OpenAIConfig) *OpenAIProvider {
...
func NewQdrantWithConfig(config *QdrantConfig, dim int) *Qdrant {
	return &Qdrant{
...
func NewVecRAGWithConfig(config *Config) (*VecRAG, error) {
...
func NewVecRAG() (*VecRAG, error) {
	...
	return NewVecRAGWithConfig(DefaultConfig())
}

func (r *VecRAG) IngestDocs(dir string, includeCode bool) (int, error) {
	chunks, err := makeChunks(dir, r.config.Indexing.ChunkSize, r.config.Indexing.ChunkOverlap, includeCode, r.config)
	if err != nil {
		return 0, err
	}
	if len(chunks) == 0 {
...
func getFileType(path string) string {
	...
	case ".go", ".py", ".js", ".ts", ".java", ".cpp", ".c", ".h", ".cs", ".php", ".rb", ".rs":
		return "code"
