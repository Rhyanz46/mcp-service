...
func Start(addr string, conf *cfg.Config, rag *ragvec.VecRAG) {
    ...
    mux.HandleFunc("/status", requireAuth(func(w http.ResponseWriter, r *http.Request) {
        ...
        var chunks *int
        if healthErr == nil {
            if c, err := q.CountPoints(); err == nil { chunks = &c }
        }
        ...
        if healthErr == nil && !fastOnly {
            ...
            for {
                ...
                if err != nil { note = fmt.Sprintf("aggregation error: %v", err); break }
                for _, pt := range pts {
                ...
                if next == nil { break }
                offset = next
                if time.Since(start) > 5*time.Second { note = "timeout: partial scan exceeded 5s"; break }
            }
            if note == "" { v := len(seen); projectsCount = &v }
        } else if fastOnly {
            note = "fast_only=true"
        }
    ...
    mux.HandleFunc("/rag/index", requireAuth(func(w http.ResponseWriter, r *http.Request) {
        ...
        if strings.TrimSpace(body.Dir) == "" { body.Dir = "./docs" }
        n, err := rag.IngestDocs(body.Dir, body.IncludeCode)
        if err != nil { writeJSON(w, http.StatusInternalServerError, errorResponse{Error: "index error", Details: err.Error()}); return }
        resp := map[string]any{
            "indexed":      n,
            "directory":    body.Dir,
    ...
    mux.HandleFunc("/rag/search", requireAuth(func(w http.ResponseWriter, r *http.Request) {
        ...
        if err := json.NewDecoder(r.Body).Decode(&body); err != nil { writeJSON(w, http.StatusBadRequest, errorResponse{Error: "invalid json", Details: err.Error()}); return }
        if strings.TrimSpace(body.Query) == "" { writeJSON(w, http.StatusBadRequest, errorResponse{Error: "query required"}); return }
        if body.K <= 0 || body.K > 20 { body.K = 5 }
